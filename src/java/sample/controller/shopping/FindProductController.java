...
public class FindProductController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            request.setAttribute("FIND_PRODUCT", search);
            ProductDAO dao = new ProductDAO();
            List<ProductDTO> listProduct = dao.getFindListProduct(search);
            if (listProduct.size() > 0) {
                int t = listProduct.size();
                request.setAttribute("FIND_LIST_PRODUCT", listProduct);
                url = SUCCESS;
            }
        } catch (Exception e) {
            log("Error at FindProductController: " + e.toString());
        } finally {
